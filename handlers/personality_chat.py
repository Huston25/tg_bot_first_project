...
async def personality_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        personality_key = query.data.replace('personality', '')
        personality = get_personality_data(personality_key)

        if not personality:
            await query.edit_message_text('К сожалению личность не найдена. Попробуй еще раз')
            return -1
        ...
        logger.error(f'Произошла ошибка при выборе личности {e}')
...
async def handle_personality_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text(personality_response)
